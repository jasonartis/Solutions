...
**Status (2026-07-10): usable end-to-end.** Full UI shipped on the live schema — single view (answer questions, see top-X matches, share-with-match reveal via the `mm_shared_answers` definer fn), matchmaker view, admin console (approval queue, question authoring, recompute). The `matchmaking.rescore` worker job now runs the same pure engine on a 30s tick, so recompute is automatic (the manual button is now just an instant-refresh convenience). Validated the "days-not-weeks" acceptance goal for the conventions. **Remaining:** the mutual-agreement→introduction flow and the conversations primitive for users→admin messaging (still doesn't exist platform-wide). Group/assignment management UI shipped 2026-07-11.
