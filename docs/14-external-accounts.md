...
| **UptimeRobot** | https://uptimerobot.com | Uptime monitoring — pings the live site every few minutes, emails a weekly report, and keeps the Supabase database from going idle. Monitor: `https://solutions-platform.vercel.app/s/pozne`, 100% uptime last week (confirmed live 2026-08-31, checklist item 1 — see docs/18). **Note: real, but the login credentials were never recorded in `.env.accounts`** — the account was created directly on uptimerobot.com and the local template's `UPTIMEROBOT_LOGIN_EMAIL`/`PASSWORD` fields are still blank placeholders. Founder to fill those in when convenient. **Also add a second monitor for `/healthz`** (docs/18 item 1's new JSON endpoint) once it's deployed — keep `/s/pozne` too, since it proves the real public page works, not just the probe. | Free account. |
